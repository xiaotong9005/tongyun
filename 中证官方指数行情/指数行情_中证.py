...
extensions='.xls'
olddir='D:\\证券分析\\中证官方指数行情\\'
newdir='D:\\证券分析\\中证官方指数行情\history\\'
codelist=['000925','000300','000001','000016','930903','000852','950090','000905','000036'
        ,'000035'
        ,'000034'
        ,'000033'
        ,'000032'
...
for code in codelist:
    ...
    with open(olddir+code+".xls",'wb') as f:
        f.write(r.content)
        print(code,'下载完成。')
...
'''-----------------------读取并写入数据库------------------------------'''
for code in codelist:
    TC_workbook=xlrd.open_workbook(olddir+code+".xls")   #打开文件
    ...
    sql='''
	   ...
	   [日期Date]
        ...
        '''  % (first_row[1],date,date,first_row[1]
               ...
               ,first_row[5],isblank(first_row[6],0),isblank(first_row[7],0)
               ,isblank(first_row[8],0),first_row[9],first_row[10]
               ,first_row[11],first_row[12],first_row[13]
               ,first_row[14],isblank(first_row[15],0),isblank(first_row[16],0)
               ,isblank(first_row[17],0),isblank(first_row[18],0))
    try:
        cur.execute(sql)
        db.commit()
        ...
        db.rollback()
        print(first_row[1],'数据提取失败,简称:',first_row[2],sql)               
    #print (first_row)
...
for code in codelist:
    try:
        os.rename(olddir+code+extensions,olddir+code+'-'+date+extensions)       #重命名
